 ...
 */
export async function uploadImage(dataUri: string, folder: string, filenamePrefix?: string): Promise<string> {
  console.log(`Storage Service: Uploading image to folder: ${folder}, Prefix: ${filenamePrefix}`);
  ...
  try {
    ...
    console.log('Storage Service: Image uploaded successfully. Snapshot:', snapshot.metadata.fullPath);
...
export async function deleteImage(downloadURL: string): Promise<void> {
     console.log(`Storage Service: Attempting to delete image with URL: ${downloadURL}`);
    if (!downloadURL || !downloadURL.includes('firebasestorage.googleapis.com')) {
        console.warn('Storage Service: Invalid or non-Firebase Storage URL provided for deletion.');
    ...
    try {
        ...
        console.log('Storage Service: Image deleted successfully from URL:', downloadURL);
    } catch (error: any) {
        ...
        if (error.code === 'storage/object-not-found') {
            console.warn(`Storage Service: Image not found for deletion (URL: ${downloadURL}). It might have already been deleted.`);
        } else {
